...
tStart = time.time()   
ada=AdalineGD(n_iter=150, eta=0.01).fit(X_std,y)
tEnd = time.time()
print("AdalineGD: time:%f"%(tEnd - tStart))
...
#決策邊界
tStart = time.time()
adaSGD=AdalineSGD(n_iter=15, eta=0.01).fit(X_std,y)
tEnd = time.time()
print("AdalineSGD: time:%f"%(tEnd - tStart))
...
plt.plot(range(1, len(ada.cost_)+1), ada.cost_,color='navy',linestyle="--",lw=2,label='AdalineGD')

#成本下降曲線圖 (AdalineGD)
plt.plot(range(1, len(adaSGD.cost_)+1), adaSGD.cost_, color='red',linestyle="--",label='AdalineSGD')
plt.legend(loc='upper left')
