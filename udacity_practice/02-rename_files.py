...
def rename_files():
	...
	#在前面加上r表示讓python知道直接讀取該路徑即可不須針對字串做處理
	file_save_path=r"02-prank"
	#執行時的資料夾
	file_path_exc="02-prank-test"
	...
	if not os.path.isdir(file_path_exc):
        #複製原始資料夾
		shutil.copytree(file_save_path, file_path_exc)
	else:
         #移除再複製原始資料夾
